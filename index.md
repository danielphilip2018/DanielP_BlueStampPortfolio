...
For my final milestone, I made the robot intelligently turn towards areas that had more space. To do this, I added 4 ultrasonic sensorsin an array, placed on top to get distance readings of all 4 directions. I then coded in a script that would make the robot used these 4 sensors to find the place with the place with the most space and go there. I expected the code to be the hardest part of this, but the challenge was actually the sensor array. The mounting was through cardboard that liked to push sensor pins and the wires away from each other. The sensors tended to give weird readings, and I had to go to the individual wires to find the issues with the code or wiring. That being said, the sensor array has helped expand the scope of this project, as even though my time is done in BSE, it opens the door to lots of much more complex ideas to be implemented. 
